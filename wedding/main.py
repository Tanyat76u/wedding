from flask import (
    ...
    request,
    redirect,
)  # импортируем фласк класс из библиотки фласк   #импортируем функцию html
from flask_sqlalchemy import (
    SQLAlchemy,
)  # класс импортируем из библиотеки flask_sqlalchemy
...
class Article(
    db.Model
):  # класс куда мы будем записывать различные статьи на сайте, наследуем все из db model - который явл обьектом SQLAlchemy
    ...
    )  # дата когда добавляется статья

    def __repr__(self):  # магический метод repr
        return (
            "<Article %r>" % self.id
        )  # когда будем выбирать какой либо обьект на основе класса, у нас будет выдаваться сам обьект и id
...
def posts():  # в артиклс первая запись из бд будет выводиться, если все надо - то all ставим
    ...
    return render_template(
        "posts.html", articles=articles
    )  # ПЕРЕДАем шаблон - такой список Артиклс, при этом в шаблоне можем иметь доступ к этому списку по названию артиклс


@app.route(
    "/create-article", methods=["POST", "GET"]
)  # отслеживаем адрес,  методы отправить и получить
def create_article():
